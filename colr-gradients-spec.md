...
A color stop is defined by a real number, the *stop offset*, and a color. A color line is defined by one or more color stops, with at least one color stop within the interval [0, 1]. Additional color stops can be specified within or outside the interval [0, 1]. (Stop offsets are represented using F2DOT14 values, therefore can only be specified within the range [-2, 2). See <span style="color:red">5.7.11.2.x</span> for format details.) If only one color stop is specified, that color is used for the entire color line; at least two color stops are needed to create color gradation.
